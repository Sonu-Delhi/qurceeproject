...
const HorizontalCardProduct = ({ category, heading }) => {
    const [data, setData] = useState([])  // Ensure data starts as an empty array
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setData(categoryProduct?.data || [])  // Default to empty array if data is undefined
        } catch (error) {
            console.error("Error fetching category products:", error)
            setData([])  // On error, set data to empty array to avoid map issues
        }
        setLoading(false)
    }
